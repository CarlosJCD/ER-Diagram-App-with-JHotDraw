...
public	class DrawApplication
	 ...
	 */
	private CH.ifa.draw.contrib.Desktop              fDesktop;
		...
		Runnable r = new Runnable() {
			public void run() {
				if (newDrawingView.isInteractive()) {
					getDesktop().addToDesktop(newDrawingView , CH.ifa.draw.contrib.Desktop.PRIMARY);
				}
				toolDone();
			}
		};
	...
	protected CH.ifa.draw.contrib.Desktop createDesktop() {
		return new JPanelDesktop(this);
//		return new JScrollPaneDesktop();
	}

	protected void setDesktop(CH.ifa.draw.contrib.Desktop newDesktop) {
	...
	*/
	public CH.ifa.draw.contrib.Desktop getDesktop() {
		return fDesktop;
	}
